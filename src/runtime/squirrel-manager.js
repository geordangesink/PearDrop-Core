...
class SquirrelManager {
  ...
  isSquirrelName(name) {
    ...
    return SQUIRREL_EXTENSIONS.has(ext);
  }
  ...
  async payloadFromPath(targetPath) {
    ...
    if (ext === ".nupkg") {
      return { type: "feed", feed: path.dirname(targetPath) };
    }
    if (ext === ".exe") return { type: "installer", file: targetPath };

    throw new Error(`unsupported Squirrel update payload: ${targetPath}`);
  }

  async apply(payloadOrPath, opts = {}) {
    const payload =
      typeof payloadOrPath === "string"
        ? await this.payloadFromPath(payloadOrPath)
        : payloadOrPath;
    if (!payload) throw new Error("missing Squirrel update payload");
    ...
    const updateExe = this.resolveUpdateExe(opts);
